    ...
    return (
        <div>
            <div className="info-title">
                <h1>영화정보</h1>
                <h3>목록 기준일 - {dayjs(date).format("YYYY년 MM월 DD일")}</h3>
            </div>
            <section className="info-grid">
                {
                    movies
                        ...
                        .map((movies, index) => {
                            ...
                            return (
                                <div key={index}>
                                    <Card title={`${movies.movieNm}`} type="inner" className="info-card">
...
// 라우터 돔으로 페이지 분할(메인/박스오피스/상세정보) -> 컴포넌트 활용여부 고민 전체영화 목록에서 페이징 처리 --> 완료했지만 알고리즘
// 공부 필요! movieNm 영화명 조회이용 --> 검색기능 구현 useState useEffect 공부 더욱더 깊이있게 해볼것! prop은
// object형식으로 컴포넌트에 전달됨 상세페이지 만들기 / css 더 공부할것 로딩화면과 결과없음 화면 구분할것